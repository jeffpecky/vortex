...
if ! release_json=$(download_file "https://api.github.com/repos/$GITHUB_REPO/releases/latest"); then
    echo "Failed to get latest release from GitHub" >&2
    exit 1
fi
...
if [ "$HAS_JQ" = true ]; then
    ...
    if [[ $release_json =~ \"tag_name\"[[:space:]]*:[[:space:]]*\"v?([0-9]+\.[0-9]+\.[0-9]+[^\"]*)\" ]]; then
        ...
        echo "Failed to parse version from GitHub API" >&2
        exit 1
    fi
fi
...
if [[ ! "$version" =~ ^[0-9]+\.[0-9]+\.[0-9]+ ]]; then
    echo "Failed to get a valid version from GitHub (got unexpected content)." >&2
    exit 1
fi

# Find binary and checksum URLs in release assets
binary_name="vortex-${platform}"
checksum_name="vortex-${platform}.sha256"

if [ "$HAS_JQ" = true ]; then
    binary_url=$(echo "$release_json" | jq -r ".assets[] | select(.name == \"$binary_name\") | .browser_download_url")
    checksum_url=$(echo "$release_json" | jq -r ".assets[] | select(.name == \"$checksum_name\") | .browser_download_url")
else
    # Extract URLs using bash regex
    if [[ $release_json =~ \"name\"[[:space:]]*:[[:space:]]*\"$binary_name\"[^}]*\"browser_download_url\"[[:space:]]*:[[:space:]]*\"([^\"]+)\" ]]; then
        binary_url="${BASH_REMATCH[1]}"
    fi
    if [[ $release_json =~ \"name\"[[:space:]]*:[[:space:]]*\"$checksum_name\"[^}]*\"browser_download_url\"[[:space:]]*:[[:space:]]*\"([^\"]+)\" ]]; then
        checksum_url="${BASH_REMATCH[1]}"
    fi
fi

if [ -z "$binary_url" ]; then
    echo "Binary $binary_name not found in release assets" >&2
    exit 1
fi

if [ -z "$checksum_url" ]; then
    echo "Checksum $checksum_name not found in release assets" >&2
    exit 1
fi
...
binary_path="$DOWNLOAD_DIR/vortex-$version-$platform"
...
# Download checksum
checksum_path="$DOWNLOAD_DIR/vortex-$version-$platform.sha256"
...
# Extract checksum from file (format: "hash  filename" or just "hash")
expected_checksum=$(head -n1 "$checksum_path" | awk '{print $1}')

if ! checksum_matches "$binary_path" "$expected_checksum"; then
